...
export default function DcaDetailPage({params}: any) {
    ...
    const satsData = data[1].map((interval: any) => ((100000000 / interval.low) * data[0].entry.amount))
    const totalSats = satsData.reduce((a: any, v: any) => a + v, 0)
    const totalInvestment = satsData.length * data[0].entry.amount
    ...
    console.log(data, totalIntervals, totalSats)
    return (
